...
function StoreRedis(args) {
  ...
  if ("undefined" == typeof args || "undefined" == typeof args.uri) {
    ...
    if (redisUrl.auth) this.password = redisUrl.auth.split(":")[1];
  } else {
...
StoreRedis.prototype.hasScript = function(sha1) {
  return this.run('script', ['exists', sha1]);
}


// #### function loadScript
// Load a script into the scripts cache, without executing it.
StoreRedis.prototype.loadScript = function(script) {
...
StoreRedis.prototype.evalSHA = function() {
  return this.run('evalsha', _.toArray(arguments));
}
