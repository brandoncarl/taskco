...
function Dispatcher(factory) {
  ...
  this.routeMessage = routeMessage.bind(this);

  // List of tasks the dispatcher is watching
  this.tasks = {};
  this.subscribed = false;
  factory.broker.getSubscriber().then(function(c) { self.subcriber = c; self = null; });
...
Dispatcher.prototype.getNextJob = function(type) {
  ...
  return this.factory.getClient().then(function(c) {
...
var routeMessage = function(channel, message) {
...
Dispatcher.prototype.broadcast = function(id, event, args) {
  ...
  return factory.getClient().then(function(c) {
