...
// Returns a promise with newly created task. If `id` is included in
// the options, factory.getNextId is not called. This helps to prevent locking
// conditions.
Tasks.create = function(factory, type, data, options) {
  ...
  if (options && options.id) {
    getNextId = when.resolve(options.id);
    delete options.id
  } else
    getNextId = factory.getNextId('task');
