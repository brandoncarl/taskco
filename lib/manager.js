...
Manager.prototype.delegate = function(team, name) {
...
Manager.prototype.getNextJob = function(name) {
  ...
  this.getClient(name, function(name, client) {
    ...
    client.blpop(self.prefix + name + ':jobs', 0, function(err) {
      // !!! RECORD ERROR HERE
      // if (err) return team.process(err);
      zpop(client, self.prefix + name + ':waiting', function(err, id){
        // !!! RECORD ERROR HERE
        // if (err) return team.process(err);
