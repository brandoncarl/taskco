...
/*
  ...
  2. The task id is added to the priority queue :tasks:email:waiting
  3. The task id is added to :tasks:email (triggering a blockPop)

  The task process goes as follows:
  ...
  3. The team asssigns a worker to process the task.
  4. As a result, the only "remaining" key in place when task is complete
     is :tasks:email:3.

  When a task is re-queued, the following occurs:
  1. The task id, with priority is added back to :tasks:email:waiting
  2. The task id is added to :tasks:email (triggering a blockPop)

  When a task is finalized, the following occurs:
  1. The tasks' metadata is updated.
  2. The tasks' uid is freed.
  3. The task is set to expire.

  When a shutdown request is made, all tasks should have their ids added
  to "purgatory". A request should be made that the tasks are "janitored",
  whereupon they will be either requeued or killed.
...
Task.prototype.save = function(client) {
  ...
  try {
    ...
    // lacksTask will return the task id if the task is found: we transform it to an error
    if (_.isFinite(err)) err = new Error("Task has conflicting uid with #" + err);
  ...
  return self.factory.execute(client, function(c) {
    return sequence(seq, c).ensure(function() {
    });
  }).ensure(cleanUp);
...
Task.prototype.queue = function(client) {
  ...
  var self = this,
      ...
      queueTask = function(c) { return c.priorityAdd(waitKey, [priority, self.id]); },
