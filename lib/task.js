...
Task.prototype.info = function(changes) {
  if ("undefined" == typeof changes)
    ...
    return this.factory.client.hashMultiSet(this.key, { metadata : this.metadata });
  }
}
...
Task.prototype.progress = function(num, den) {
  ...
  if ("undefined" == typeof num)
    ...
    if ("undefined" == typeof den)
      this.metadata.progress = Math.min(num, 100);
    else
      this.metadata.progress = Math.min(100 * num / den, 100);

    promise = client.hashMultiSet(key, { metadata : this.metadata });
...
Task.prototype.finalize = function(state, progress) {
  ...
  promise = client.hashMultiSet(key, { metadata : this.metadata })
                  .then(function() {
                    ...
                    return client.hashUnset(uidKey, [uid]);
                  })
                  .then(expireFxn);
...
Task.prototype.save = function() {
  ...
  promise = uidFxns.check()
            .then(function() { return self.factory.client.hashMultiSet(taskKey, self.serialize()); })
            .then(function() { return self.factory.client.sortedAdd(waitKey, [priority, self.id]); })
            .then(uidFxns.update)
            .then(function() { return self.factory.client.push(typeKey, [self.id]); })
...
privates.uidFunctions = function(task) {
  ...
  if ("undefined" != typeof task.uid) {
    ...
    fxns.update = function() { return task.factory.client.hashSet(uidKey, task.uid, task.id); };
  } else {
...
privates.expireFunction = function(task) {
  ...
  if ("undefined" == typeof expires)
    ...
    return function() { return task.factory.client.expire(key, expires); };
}
