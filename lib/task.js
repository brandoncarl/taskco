...
// #### function metadata
// Overloaded operator.
Task.prototype.metadata = function(changes) {
  ...
  //     proxy.run(this.factory.client, "hmset", this.key, { metadata : this.metadata });
  // }
}
...
privates.uidFunctions = function(task) {
  ...
  if ("undefined" != typeof task.uid) {
    ...
    fxns.update = function() { return proxy.run(task.factory.client, 'sadd', uidKey, task.id); };
  } else {
