...
// Loads all scripts from lua directory.
Factory.prototype.loadScripts = function() {

  var self = this;

  return this.execute(function(c) {
    var promises = _.map(Scripts.scripts, function(script, name) {
      if ("undefined" != typeof self.scripts[name]) return when.resolve();

      return c.hasScript(Scripts.shas[name]).then(function(present) {
        // If script is present abandon execution
        if (1 == present[0]) return when.resolve();
        return c.loadScript(script);
      })

      .then(function() {
        self.scripts[name] = Scripts.shas[name];
        return when.resolve();
      });
    });

    return when.all(promises).ensure(function() { self = null; });
  });
...
function shutdown(timeToLive) {
  ...
  shutdownTeams = function() {
    var teamShutdowns = _.map(self.teams, function(team) {
      return team.shutdown(timeToLive);
    });
  ...
  cleanup = function() {
    _.each(self.teams, function(team) { team.removeAllListeners(); });
...
Factory.prototype.getNextId = function(name, client) {

  var key = this.prefix + 'counters';

  return this.execute(client, function(c) {
...
Factory.prototype.getTaskId = function(type, uid) {
  return Tasks.findId(this, type, uid);
}
...
Factory.prototype.quickEntry = function(type, data, options) {
  ...
  return this.execute(null, function(c) {
    return self.getNextId('task', c)
               .then(function(id) {
                 return self.createTask(type, data, _.extend(options || {}, { id : id }))
               })
               .then(function(t) {
                 task = t;
                 return task.save(c);
...
Factory.prototype.lacksTask = function(type, uid, client) {
  ...
  Tasks.findExists(this, type, uid, client).then(function(id) {
    deferred.reject(id);
  }, function(err) {
